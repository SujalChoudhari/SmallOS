...
typedef struct {
  ...
  uint16_t total_sectors;           // Total number of sectors (or 0 if using
                                    // large_sector_count)
  ...
  uint32_t large_sector_count;      // Total number of sectors (for FAT32 or if
                                    // total_sectors == 0)
...
typedef struct {
  uint8_t name[11];   // File name (8.3 format: 8 characters name, 3 characters
                      // extension)
...
directory_entry_t *g_root_directory =
    NULL;                      // Global variable for the root directory entries
...
// Reads a specified number of sectors from the disk image starting at LBA into
// a buffer
bool read_sectors(FILE *disk, uint32_t lba, uint32_t count, void *buffer_out) {
  ...
  ok = ok && (fread(buffer_out, g_boot_sector.bytes_per_sector, count, disk) ==
              count);
  return ok;
}
...
bool read_fat(FILE *disk) {
  g_fat = (uint8_t *)malloc(g_boot_sector.sectors_per_fat *
                            g_boot_sector.bytes_per_sector);
  return read_sectors(disk, g_boot_sector.reserved_sectors,
                      g_boot_sector.sectors_per_fat, g_fat);
}

// Reads the root directory entries from the disk image into the global
// `g_root_directory`
bool read_root_directory(FILE *disk) {
  ...
  g_root_directory =
      (directory_entry_t *)malloc(sectors * g_boot_sector.bytes_per_sector);
...
// Reads the contents of a file specified by the `directory_entry_t` into a
// buffer
bool read_file(directory_entry_t *file_entry, FILE *disk,
               uint8_t *output_buffer) {
  bool ok = true;
  uint16_t current_cluster = file_entry->first_cluster_low;

  do {
    uint32_t lba = g_root_directory_end +
                   (current_cluster - 2) * g_boot_sector.sectors_per_cluster;
    ok = ok && read_sectors(disk, lba, g_boot_sector.sectors_per_cluster,
                            output_buffer);
    output_buffer +=
        g_boot_sector.sectors_per_cluster * g_boot_sector.bytes_per_sector;
...
int main(int argc, char **argv) {
  ...
  uint8_t *buffer =
      (uint8_t *)malloc(file_entry->size + g_boot_sector.bytes_per_sector);
